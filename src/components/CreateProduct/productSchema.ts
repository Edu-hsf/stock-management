...
const schema = z.object({
    ...
    price: z.coerce.number().max(1000000000, 'Maximum size reached.').refine(val => val >= 0, { message: 'The value cannot be negative.' }).optional(),
...
}).refine(val => val.stockMin && val.stockMax && val.stockMin <= val.stockMax, {
    message: 'The minimum stock must be less than the maximum stock.',
    path: ['stockMin']
}).refine(val => val.stockMax && val.stockMax >= val.quantity || val.stockMax == 0, {
    message: 'The current quantity cannot exceed the maximum stock.',
    path: ['quantity']
}).refine(val => val.stockMin && val.stockMin <= val.quantity || val.stockMin == 0, {
