...
import { emailIsValidAction } from "@/services/actions/emailAction"
...
export default function SignUp({ setMove_x, setMove_y }: MoveType) {
    ...
    const registerUser: SubmitHandler<SignUpType> = async (data) => {
        ...
        const emailIsValid = await emailIsValidAction(newUser.email)
        ...
        if (emailIsValid) {
            ...
            setError("email", { type: "manual", message: "This email does not exist." })
            setLoading(false)
        }
    }
    ...
    return (
        <div className="sign-up">
            <form onSubmit={handleSubmit(registerUser)} className="sign-up-form">
                ...
                <button
                    type="submit"
                    className="w-100 btn btn-light-green mt-4 d-flex justify-content-center align-items-center"
